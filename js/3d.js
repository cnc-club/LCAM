...
function canvas3d(){
			...
			function init() {
				...
				// CAMERA
				camera = new THREE.PerspectiveCamera( 10, cw() / ch(), 1, 80000 );
			...
			function createNewTeapot() {
				...
				var material = new THREE.MeshBasicMaterial( { transparent: true, opacity: 0.5 ,color: 0xffff00 } );
				var lathe = new THREE.Mesh( geometry, phongMaterial );
				scene.add( lathe );
				
				var axisHelper = new THREE.AxisHelper( 5 );
				scene.add( axisHelper );

				edges = new THREE.EdgesHelper( lathe, 0xaa0000 );
				scene.add( edges );
